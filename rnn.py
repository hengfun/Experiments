...
def gru_ortho_initializer(scale=1.0):
  """gru orthogonal initializer."""
  def _initializer(shape, dtype=tf.float32,
                   partition_info=None):  # pylint: disable=unused-argument
    size_x = shape[0]
    size_h = shape[1] // 4  # assumes lstm.
...
class GRUCell(tf.contrib.rnn.RNNCell):
  ...
  def __call__(self, x, h, scope=None):
    with tf.variable_scope(scope or type(self).__name__):
      ...
      # Keep W_xh and W_hh separate here as well to use different init methods.
      w_xh = tf.get_variable(
          'W_xh', [x_size, 2 * self.num_units], initializer=w_init)
      w_hh = tf.get_variable(
          'W_hh', [self.num_units, 2 * self.num_units], initializer=h_init)
      bias = tf.get_variable(
          'bias', [2 * self.num_units],
          initializer=tf.constant_initializer(0.0))

      w2_xh = tf.get_variable(
        'W_xh2', [x_size, 1 * self.num_units], initializer=w_init)
      w2_hh = tf.get_variable(
        'W_hh2', [self.num_units, 1 * self.num_units], initializer=h_init)
      bias2 = tf.get_variable(
        'bias2', [1 * self.num_units],
        initializer=tf.constant_initializer(0.0))
      ...
      z, r = tf.split(hidden, 2, 1)
      # concat2 = tf.concat([])
      h_candidate=tf.tanh(tf.matmul(x,w2_xh)+tf.matmul(tf.multiply(r,h),w2_hh)+bias2)


      new_h = tf.multiply(z,h) + tf.multiply((1.0 - z),h_candidate)
