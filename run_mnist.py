...
class MNISTModel(object):
    def __init__(self,args):
        ...
        self.X = tf.placeholder(dtype=self.dtype,shape=[None,28*28])
        ...
        output, last_state = tf.nn.dynamic_rnn(self.cell,self.X_,dtype=self.dtype)
        # self.last_hidden = output[:,-1,:]
        ...
        correct_pred = tf.equal(self.prediction, tf.cast(tf.argmax(self.Y, 1),tf.int32))
